...
# scikitlearnのrandom-forestの自習
# probalility calibration for 3-class classificatin
...
# trainデータ、validationデータをクラスわけし、テストデータを価値付ける
clf = RandomForestClassifier(n_estimators=25)
clf.fit(X_train_valid, y_train_valid)
clf_probs = clf.predict_proba(X_test)
score = log_loss(y_test, clf_probs)

# ランダムフォレストを実行、validataionデータで測定しテストデータを価値付ける
clf = RandomForestClassifier(n_estimators=25)
...
print(" * classifier trained on 600 datapoints and calibrated on " "200 datapoint: %.3f" % sig_score)

# 測定をイラストにする
plt.figure(1)

# 2つのシンプレックスにわたるグリッドを書く
...
# plot modification of calibrator
# 測定の変更をプロットする
...
plt.title("Illustration of simoid calibrator")
plt.xlabel("Probability class 1")
plt.ylabel("Probability class 2")
plt.xlim(-0.05, 1.05)
plt.ylim(-0.05, 1.05)
